...
def get_entity(model,tokenizer,sentence,max_len = 64):
    ...
    # encode_plus返回所有的编码信息，具体如下：input_ids:是单词在词典中的编码 token_type_ids’:区分两个句子的编码（上句全为0，下句全为1） attention_mask’:指定对哪些词进行self-Attention操作
...
def select_database(sql):
    # connect database
    connect = pymysql.connect(user="root",password="12345",host="127.0.0.1",port=3306,db="kb_qa",charset="utf8")
...
def main():
    ...
    with torch.no_grad():
        ...
        ner_model = get_ner_model(config_file = './input/config/bert-base-chinese-config.json',
                                  pre_train_model = './output/best_ner1.bin',label_num = len(ner_processor.get_labels()))
